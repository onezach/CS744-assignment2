...
part() {
    ...
    ssh -p "$worker_port" "$HOST_NAME" "cd ~/CS744-assignment2/main/part$part_id && ./run-p$part_id.sh $rank" > "${LOG_DIR}/part${part_id}/rank${rank}.log" 2>&1 &
    echo "Started part$part_id with rank $rank on $HOST_NAME:$worker_port"
}

part1() {
    echo "Starting part1"
    cd ~/CS744-assignment2/main/part1 && ./run-p1.sh 0 > "${LOG_DIR}/part1/log" 2>&1
}
...
main() {
    ...
    for part_id in "${PART_IDS[@]}"; do
        ...
        cd ~/CS744-assignment2/main/part$part_id && ./run-p$part_id.sh 0 > "${LOG_DIR}/part${part_id}/rank0.log" 2>&1 & # Run the master process
        rank=1
